...
def extract_keywords(text: str) -> List[str]:
    ...
    stopwords = {
        ...
        "looking", "candidate", "candidates", "skills", "skill", "experience", "preferred",
        "required", "years", "year", "month", "months", "day", "days", "etc", "new", "best",
        "need", "plus", "get", "build", "make", "help", "across", "within", "ability",
        "including", "responsible", "knowledge", "understanding", "seeking", "seeks",
        "graduate", "graduates", "fresher", "freshers"
    }
...
def call_llm(system_prompt: str, user_prompt: str) -> str:
    ...
    if client is None:
        raise RuntimeError("OpenAI client not available. Add OPENAI_API_KEY in Streamlit secrets and turn on the API toggle.")
...
def build_resume_fallback(name, email, phone, location, target_role, profile_type, summary, education, experience, skills, projects, certifications):
    default_summary_map = {
        "No experience": f"Motivated fresher targeting {target_role or 'business'} roles with strong learning agility, communication skills, and a structured approach to problem-solving.",
        "Internship only": f"Early-career candidate targeting {target_role or 'business'} roles with internship exposure, business understanding, and the ability to execute in fast-paced environments.",
        "Career switch": f"Professional transitioning into {target_role or 'business'} roles with transferable skills, adaptability, and a strong willingness to learn and deliver results.",
        "Experienced": f"Results-oriented candidate targeting {target_role or 'business'} roles with relevant execution experience, stakeholder handling, and problem-solving skills.",
    }
    return f"""# {name or 'Your Name'}
{email or 'email@example.com'} | {phone or '+91XXXXXXXXXX'} | {location or 'Your City'}
...
def build_cover_letter_fallback(name, company, role, summary, strengths, jd_text):
    keywords = extract_keywords(jd_text)[:6]
    kw_text = ", ".join(keywords) if keywords else "communication, execution, and adaptability"
    return f"""Dear Hiring Manager,

I am writing to express my interest in the {role or 'target'} role at {company or 'your company'}. I am excited about this opportunity because it aligns with my background and my interest in building a strong career in this field.
...
def score_resume(resume_text, jd_text, profile_type, skills_text, experience_text, projects_text, target_role):
    score = 35
    feedback = []
    positives = []

    skills_count = len([s for s in skills_text.split(",") if s.strip()])
    exp_lines = len([x for x in experience_text.splitlines() if x.strip()])
    proj_lines = len([x for x in projects_text.splitlines() if x.strip()])
...
def generate_action_plan(target_role, missing_skills, resume_score, profile_type):
...
def save_current_version(name, company, target_role):
    entry = {
...
with c_top2:
    if st.button("Clear All", use_container_width=True):
        user_keys = [
            "name", "email", "phone", "location", "target_role", "profile_type", "summary", "education",
            "experience", "projects", "certifications", "skills", "company", "jd_text"
        ]
...
with st.sidebar:
    st.header("Settings")
    use_ai = st.toggle("Use OpenAI API", value=False, help="Turn on only after setting OPENAI_API_KEY in Streamlit secrets.")
    ...
    if history:
        ...
        selected_label = st.selectbox("Compare past versions", labels)
...
with left:
    ...
    current_profile = st.session_state.get("profile_type", "No experience")
    profile_type = st.selectbox("Profile Type", profile_options, index=profile_options.index(current_profile) if current_profile in profile_options else 0)
    summary = st.text_area("Professional Summary", value=st.session_state.get("summary", ""), placeholder="Recent PGDM graduate targeting sales and account management roles with strong communication, market understanding, and learning agility.", height=110)
    education = st.text_area("Education", value=st.session_state.get("education", ""), placeholder="PGDM, IMT Nagpur\nFood Technology degree, Institute Name", height=90)
    experience = st.text_area("Experience / Internships / Responsibilities", value=st.session_state.get("experience", ""), placeholder="Handled...\nAnalyzed...\nLed...\nImproved...", height=140)
    projects = st.text_area("Projects", value=st.session_state.get("projects", ""), placeholder="Built a market entry analysis...\nCreated a customer persona framework...", height=110)
    certifications = st.text_area("Certifications", value=st.session_state.get("certifications", ""), placeholder="Google Analytics\nAdvanced Excel\nSales training", height=80)
    skills = st.text_area("Skills (comma-separated)", value=st.session_state.get("skills", ""), placeholder="Excel, PowerPoint, Sales, Communication, Market Research, Python", height=90)
    st.subheader("Target Job")
    company = st.text_input("Company Name", value=st.session_state.get("company", ""), placeholder="Zomato")
    jd_text = st.text_area("Paste Job Description", value=st.session_state.get("jd_text", ""), placeholder="Paste the full job description here...", height=220)

with right:
    c_save1, c_save2 = st.columns([1, 3])
    with c_save1:
        if st.button("Save Current Version", use_container_width=True):
            save_current_version(name=name, company=company, target_role=target_role)
...
"""
        ...
        if resume_output:
            ...
            with col_a:
                st.download_button("Download Resume as .md", data=resume_output, file_name="resume.md", mime="text/markdown", use_container_width=True)
            with col_b:
                if REPORTLAB_AVAILABLE:
                    st.download_button("Download Resume as PDF", data=make_pdf_bytes("Resume", resume_output), file_name="resume.pdf", mime="application/pdf", use_container_width=True)
                else:
    ...
    with tabs[1]:
        ...
        if st.button("Auto Improve My Resume", use_container_width=True):
            ...
            try:
                if use_ai:
                    prompt = f"""
Improve the following resume for a fresher or early-career candidate targeting {target_role}.
...
"""
        ...
        if cover_output:
            ...
            with col_c:
                st.download_button("Download Cover Letter as .txt", data=cover_output, file_name="cover_letter.txt", mime="text/plain", use_container_width=True)
            with col_d:
                if REPORTLAB_AVAILABLE:
                    st.download_button("Download Cover Letter as PDF", data=make_pdf_bytes("Cover Letter", cover_output), file_name="cover_letter.pdf", mime="application/pdf", use_container_width=True)
                else:
                    st.caption("Install reportlab for PDF export.")
...
Answer: Connect your skills and willingness to learn with the job."""
        ...
        st.text_area("Interview Questions & Answers", value=interview_output, height=300)
...
"""
                    feedback_out = call_llm("You are an interview coach.", eval_prompt)
                else:
                    feedback_out = "Your answer is decent. Improve structure using the STAR method and be more specific."
                st.session_state["interview_feedback"] = feedback_out
            except Exception as e:
...
with st.expander("Add New Application", expanded=False):
    ...
    app_date = st.date_input("Date Applied", key="app_date", value=date.today())
    follow_up = st.date_input("Follow-up Date", key="follow_up", value=date.today())
...
if apps:
    ...
    header[3].markdown("**Applied**")
    header[4].markdown("**Follow-up**")
    header[5].markdown("**Del**")
    for i, app in enumerate(apps):
        row = st.columns([2, 2, 2, 2, 2, 1])
        row[0].write(app["company"])
        row[1].write(app["role"])
        row[2].write(app["status"])
        row[3].write(app["date"])
        row[4].write(app["follow_up"])
        with row[5]:
...
if st.button("Get Smart Suggestions", use_container_width=True):
    score = st.session_state.get("resume_score") or 0
    match = st.session_state.get("match_score") or 0
    suggestions_out = []
    if score < 60:
        suggestions_out.append("Focus on improving your resume before applying more.")
    if match < 50:
        suggestions_out.append("You are applying to mismatched roles. Narrow your targeting.")
    if len(apps) < 10:
        suggestions_out.append("Increase application volume to at least 10 to 15 per week.")
    if len([a for a in apps if a["status"] == "Interview"]) == 0 and len(apps) > 5:
        suggestions_out.append("Your resume is not converting. Rewrite key bullet points.")
    if not suggestions_out:
        suggestions_out.append("You are on the right track. Keep applying and refining.")
    st.session_state["smart_suggestions"] = suggestions_out
for s in st.session_state.get("smart_suggestions", []):
    st.write(f"- {s}")
...
feedback_text = st.text_area("Share your feedback", placeholder="What did you like, what felt confusing, what should be improved?", height=140, key="feedback_text")
...
if logs:
    ...
    f1.metric("Total Feedback", len(logs))
    f2.metric("Average Rating", f"{avg_rating}/5")
    f3.metric("Would Recommend", f"{recommend_pct}%")
...
st.caption("Tip: Add OPENAI_API_KEY in Streamlit secrets to enable AI generation.")
